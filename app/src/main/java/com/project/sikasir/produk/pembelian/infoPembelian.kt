...
class infoPembelian : Fragment() {
    val listProduk = ArrayList<classPembelian>()
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val dbref = FirebaseDatabase.getInstance().getReference("Pembelian")
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    ...
                    for (snap in snapshot.children) {
                        val beli = snap.getValue(classPembelian::class.java)
                        listProduk.add(beli!!)
                    }
                    view.rv_pegawai_kp.adapter = adapterInformasiPembelian(listProduk)
                } else {
